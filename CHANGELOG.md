# Changelog

## [3.0.0](https://github.com/ScottGibb/Home-Lab-Containers/compare/v2.2.0...v3.0.0) (2026-09-17)


### ⚠ BREAKING CHANGES

* replace motioneye with frigate
* deprecate Pi Zero scripts as didnt work in the end
* Migrate to better samba container
* remove PiDesk and Incorporate into PiHall
* add PiHall

### Features

* add balcony DS18B20 temperature sensor ([4e5f757](https://github.com/ScottGibb/Home-Lab-Containers/commit/4e5f75739c7486587cf43d8a87efa3c8bcd0a40a))
* add balcony light on multiplexer channel 2 ([363199e](https://github.com/ScottGibb/Home-Lab-Containers/commit/363199e0bdc7d464de403de22bfaecf2af50683f))
* add Detector System ([e6a0398](https://github.com/ScottGibb/Home-Lab-Containers/commit/e6a0398853d8ba853581a00e3f35c8ee6e34c326))
* add docker compose plugin for linting ([6f1d4dc](https://github.com/ScottGibb/Home-Lab-Containers/commit/6f1d4dc986053ddf71f8da184f82fb0f4e59a57e))
* Add ESPHome and Outdoor Watering System ([2617006](https://github.com/ScottGibb/Home-Lab-Containers/commit/26170060f0f9384ce96ffe33b832193d699cf3a7))
* add fstab for PiHall ([2563967](https://github.com/ScottGibb/Home-Lab-Containers/commit/256396734712d74d4cfb3e53436eeb9420cb938d))
* add full hardware with new pinout ([d6bd51d](https://github.com/ScottGibb/Home-Lab-Containers/commit/d6bd51d1ccc82e82bb1d629c567e8ceff02975a7))
* add Home Power Monitor ([a2650ee](https://github.com/ScottGibb/Home-Lab-Containers/commit/a2650eef42fe994d83c55ac2b22cbe75dcac9296))
* add missing nginx.conf ([d42e247](https://github.com/ScottGibb/Home-Lab-Containers/commit/d42e2479d83ff1ccad2ebdf0255560e07c6b2620))
* add Moisture Sensing and Fix pinout ([8e0b670](https://github.com/ScottGibb/Home-Lab-Containers/commit/8e0b670f6e0311f04d6c0a76d3daccfd9fd8cf30))
* add MQTT presence settings ([82fdac6](https://github.com/ScottGibb/Home-Lab-Containers/commit/82fdac62554a51a3fb1640d85c907cf8cb9f9f15))
* add Node-Red Config ([3642708](https://github.com/ScottGibb/Home-Lab-Containers/commit/364270839d37f24d44ebb8d3d08ea3560663d235))
* add node-red persistent data ([e7e2578](https://github.com/ScottGibb/Home-Lab-Containers/commit/e7e257800216fbe00595210536500da3c5550e6e))
* Add PiForwarder ([54f7c6f](https://github.com/ScottGibb/Home-Lab-Containers/commit/54f7c6fd3663eee45b880fb4d213e6447a14ad5f))
* add PiForwarder PiZero Variant ([6ac27f9](https://github.com/ScottGibb/Home-Lab-Containers/commit/6ac27f92afee6f42af5d3db1115c5b4221237234))
* add PiHall ([b6d5b46](https://github.com/ScottGibb/Home-Lab-Containers/commit/b6d5b46b233b95683df30a15c7b9eb76dbc4f6d1))
* add vscode settings for formatters ([b16d135](https://github.com/ScottGibb/Home-Lab-Containers/commit/b16d1353c5064858b52c0d8bbd762ae7b50d2b56))
* add water tank percentage ([a6c2be4](https://github.com/ScottGibb/Home-Lab-Containers/commit/a6c2be410b979c04a06d05d58439db8d4b243f84))
* bump vers ([4ad78a4](https://github.com/ScottGibb/Home-Lab-Containers/commit/4ad78a4c4923c76ddccbc938ab6778bf780a9bae))
* deprecate Pi Zero scripts as didnt work in the end ([969b6fd](https://github.com/ScottGibb/Home-Lab-Containers/commit/969b6fdfa440db6491906575f87d808a0c247188))
* Migrate to better samba container ([5c9e992](https://github.com/ScottGibb/Home-Lab-Containers/commit/5c9e992a753078359328a4cd9ebbaa58df68a2f6))
* migrate to latest Power Monitor ([aaef98f](https://github.com/ScottGibb/Home-Lab-Containers/commit/aaef98f225521630c0f283391d85af16a7fe4c40))
* remove PiDesk and Incorporate into PiHall ([c08f147](https://github.com/ScottGibb/Home-Lab-Containers/commit/c08f147fca101af2fac4bc1ca9f26747cecb54b9))
* remove submodule config ([c27e24f](https://github.com/ScottGibb/Home-Lab-Containers/commit/c27e24f83bb8a898c682db4e5310c70f4078e32f))
* replace motioneye with frigate ([145e444](https://github.com/ScottGibb/Home-Lab-Containers/commit/145e444cd774a83c24f4c4975c2a1bbc09366a4d))


### Bug Fixes

* add .gitignore for MacOS ([7cc33a2](https://github.com/ScottGibb/Home-Lab-Containers/commit/7cc33a2a956f8ecb87f5eadcb3bb20eae6ae93b4))
* adjust wet value ([3a90870](https://github.com/ScottGibb/Home-Lab-Containers/commit/3a90870776ee71788a510abd2640ab40fc10c55e))
* config folder ([58ce8c2](https://github.com/ScottGibb/Home-Lab-Containers/commit/58ce8c28c166f7655804b9683cf9dcd80c369ed0))
* dclint linting errors: add interface prefixes to ports and concrete image version tags ([2b9785e](https://github.com/ScottGibb/Home-Lab-Containers/commit/2b9785e536452e1968cf0954ad44db9ba26c96f0))
* dependabot submodule issues ([58f32a9](https://github.com/ScottGibb/Home-Lab-Containers/commit/58f32a99c7096d85ac0ba8fb6923d98a8dc9681c))
* docker 64 bit issue ([c179f9d](https://github.com/ScottGibb/Home-Lab-Containers/commit/c179f9ddeffd25eada02fe179d52878a32af9ebb))
* docker compose ([2864cbf](https://github.com/ScottGibb/Home-Lab-Containers/commit/2864cbfcb1bb90c2b5995d8a83f732218e9b7be0))
* Docker compose format ([c30398f](https://github.com/ScottGibb/Home-Lab-Containers/commit/c30398fb7f8d3b85e38bf63b39e3ea482581c01c))
* docker script ([fa8bc45](https://github.com/ScottGibb/Home-Lab-Containers/commit/fa8bc450e9576376b9ad191f63edaf1cab97176e))
* **docs:** Dependabot URL configuration in README ([881e085](https://github.com/ScottGibb/Home-Lab-Containers/commit/881e085ca39fdecaff904063bfc36bcd47fb58f1))
* **docs:** remove redudant docs ([d2d66d4](https://github.com/ScottGibb/Home-Lab-Containers/commit/d2d66d4b7d71010feaef4f4a1d541825ae85bb44))
* **docs:** typo ([0d94730](https://github.com/ScottGibb/Home-Lab-Containers/commit/0d947304b7b7f4b6fad5ab440f32bc755386a31f))
* ESDP Mount Path ([224afaa](https://github.com/ScottGibb/Home-Lab-Containers/commit/224afaab5f51a4dbb6ebf72ff575bcf82ffde1fa))
* ESPHome pin numbering ([4891174](https://github.com/ScottGibb/Home-Lab-Containers/commit/48911743e579a85df2145041e3eee861bbb2dbf4))
* folder paths ([86f77bc](https://github.com/ScottGibb/Home-Lab-Containers/commit/86f77bc8cf0a1a19a298632d29d5d397c11ab4da))
* found the best pinout! ([bbf55bd](https://github.com/ScottGibb/Home-Lab-Containers/commit/bbf55bd64f809a70c0a17956bfaaefba827add08))
* Home Power Monitor Config ([4a86287](https://github.com/ScottGibb/Home-Lab-Containers/commit/4a862874270d03e5e6a6e43013f5953ea1c8acb2))
* HPM config ([5d107d8](https://github.com/ScottGibb/Home-Lab-Containers/commit/5d107d883f2912b7d43c63889ede767c31f3fcd9))
* increase sampling time ([ef07342](https://github.com/ScottGibb/Home-Lab-Containers/commit/ef073422ce00d754ba08c6b2c34cff7e16208b9d))
* Increase sampling time ([47d7bb0](https://github.com/ScottGibb/Home-Lab-Containers/commit/47d7bb00f8b27c83ea778ddbbcd08880253187bc))
* Installation scripts ([890da26](https://github.com/ScottGibb/Home-Lab-Containers/commit/890da26cbe72cda04a29c7ff743def5383f51fb5))
* missing units ([a8cbd7b](https://github.com/ScottGibb/Home-Lab-Containers/commit/a8cbd7b6f0ef2db059f997e5682796b8505295a4))
* most up to date power monitor ([1709458](https://github.com/ScottGibb/Home-Lab-Containers/commit/1709458d4866d9091616d2965dd18e7d11e5d1d7))
* mux numbering for light snsors ([2735d42](https://github.com/ScottGibb/Home-Lab-Containers/commit/2735d42e4cdb9a90e5c19289ce97a820f9b8552b))
* networking on pihome ([60ab9a5](https://github.com/ScottGibb/Home-Lab-Containers/commit/60ab9a54cde3ad1f0823d9424cdf28b4b327e39b))
* pihall dns ([335ea88](https://github.com/ScottGibb/Home-Lab-Containers/commit/335ea889652971f1724cfb667544ee2a850d81e7))
* PiHall Router ([2243998](https://github.com/ScottGibb/Home-Lab-Containers/commit/2243998fc3a88cf5343dd34e0c2f11c2d0916dad))
* pin numberings ([31362ec](https://github.com/ScottGibb/Home-Lab-Containers/commit/31362ec8dabc3a1e9b8ec1cfa0dc0a1c97892dd5))
* pin wiring ([bb23b8b](https://github.com/ScottGibb/Home-Lab-Containers/commit/bb23b8b391897322a2deaa119d83551d35b349b8))
* Pinning Dockerfile versions ([21fe9a4](https://github.com/ScottGibb/Home-Lab-Containers/commit/21fe9a4fedc46b1abeb7da40467d553be3d857f5))
* Possibly fix serial issue ([06adfaf](https://github.com/ScottGibb/Home-Lab-Containers/commit/06adfaf15b291246f37ba0427401b07a8e9393e8))
* remove bind mount ([9a30e16](https://github.com/ScottGibb/Home-Lab-Containers/commit/9a30e162e49bcf76e3b9a4851b6d0c44b69d3bca))
* rename watering system ([2c552e0](https://github.com/ScottGibb/Home-Lab-Containers/commit/2c552e0bf7059ecfbcc91faf5789f6b675edcd0f))
* revert previous change, LED on WEMO board causes presence issues ([01dbd46](https://github.com/ScottGibb/Home-Lab-Containers/commit/01dbd46f95e94f24d8e19029f118917e2d733f94))
* rm ftstab ([0faebdb](https://github.com/ScottGibb/Home-Lab-Containers/commit/0faebdb3d8d1ff53152f658a91010abdae7831d4))
* Samba ([12720b4](https://github.com/ScottGibb/Home-Lab-Containers/commit/12720b44df3de82c3a8b7ccd2d85cb23433cceac))
* sha ([21540a6](https://github.com/ScottGibb/Home-Lab-Containers/commit/21540a6831dfe02486883d6b7406ea8dbf57f8e2))
* tagged versions for docker compose ([f14c549](https://github.com/ScottGibb/Home-Lab-Containers/commit/f14c5492e6287d0ee5458df185e416f8d4974f99))
* Update dependabot.yaml to remove directories ([cfcae3d](https://github.com/ScottGibb/Home-Lab-Containers/commit/cfcae3dd36dfb2ee0d5dc3fc05c07b466252fffe))
* update docker compose for HPM ([627d98a](https://github.com/ScottGibb/Home-Lab-Containers/commit/627d98a2a0322ed3045b5c4ec19afbf4515beb8a))
* update power monitor version ([a6d1a6e](https://github.com/ScottGibb/Home-Lab-Containers/commit/a6d1a6e8944dcb3ef20a92904bc0bf76baa3cb78))
* update samba ([53439de](https://github.com/ScottGibb/Home-Lab-Containers/commit/53439dee683d1ad4b229272f76d6955f32a12686))
* update sampling time ([9563f0d](https://github.com/ScottGibb/Home-Lab-Containers/commit/9563f0d295b14a7335f8494ef96bfe22e7284a10))
* water level sensor now detects distance ([22b120e](https://github.com/ScottGibb/Home-Lab-Containers/commit/22b120eb589798dc6c2cccbec5bb0c95df059c18))
* Web Based Fan Controller ([0fa30d1](https://github.com/ScottGibb/Home-Lab-Containers/commit/0fa30d154d7c0693c070e7c47674578aaa05a47c))
* wifi connect removal ([e5baa3c](https://github.com/ScottGibb/Home-Lab-Containers/commit/e5baa3c65c608bdbbdab2d3ca6232d0c92a78b9a))

## [2.2.0](https://github.com/ScottGibb/Home-Lab-Containers/compare/v2.1.1...v2.2.0) (2026-02-18)

### Features

* Add name attribute to all subdirectory docker-compose files ([a587178](https://github.com/ScottGibb/Home-Lab-Containers/commit/a58717875e7eb00ad8c4b381346b6d96581e4c7b))
* Create main docker-compose files with include directives and inline env vars ([a40643b](https://github.com/ScottGibb/Home-Lab-Containers/commit/a40643b98a8d2bfe3a0cf56c7602c0f50d92d761))
* migrate to one nginx ([c861adf](https://github.com/ScottGibb/Home-Lab-Containers/commit/c861adf5d0bb2a13482fdc2c116a320573b2ed8b))
* Remove all bash scripts and .env files ([e958e30](https://github.com/ScottGibb/Home-Lab-Containers/commit/e958e30f4d801da6afeba632e5822dba418eee58))

### Bug Fixes

* add nas stack ([b28a81d](https://github.com/ScottGibb/Home-Lab-Containers/commit/b28a81d3cfb57d4a62265469a9b76f7bd3f2e443))
* case sensitivity in Dependabot exclude paths for private submodule ([f555f7b](https://github.com/ScottGibb/Home-Lab-Containers/commit/f555f7b0b5acd99163ad70195094c7bf15d0198e))
* **docs:** README ([6dac1e7](https://github.com/ScottGibb/Home-Lab-Containers/commit/6dac1e75f8e4c0984c6e8f3f609f4c7c568e1a11))
* Gitlab version ([43897d3](https://github.com/ScottGibb/Home-Lab-Containers/commit/43897d3c2b94fe71a2a7fd3030c34a2123a0976f))
* make portainer data persistent ([0ec9f03](https://github.com/ScottGibb/Home-Lab-Containers/commit/0ec9f038bfc5a461d2a7d3f99c99c8850d16e5d7))

## [2.1.1](https://github.com/ScottGibb/Home-Lab-Containers/compare/v2.1.0...v2.1.1) (2026-02-08)

### Bug Fixes

* nginx ip ([f067775](https://github.com/ScottGibb/Home-Lab-Containers/commit/f06777531c43e3280fdf7d85fbc4e8babbe59524))
* pin nginx ([8d93c3a](https://github.com/ScottGibb/Home-Lab-Containers/commit/8d93c3aee92f8fdc8a4ce6c10e40acb52aabdd83))

## [2.1.0](https://github.com/ScottGibb/Home-Lab-Containers/compare/v2.0.0...v2.1.0) (2026-02-08)

### Features

* add pinned utils ([dcdfde0](https://github.com/ScottGibb/Home-Lab-Containers/commit/dcdfde03d0b80845646c937e8aa5c14832b31d6b))

### Bug Fixes

* use pinned image for gitlab ([dc7700a](https://github.com/ScottGibb/Home-Lab-Containers/commit/dc7700a1607fa1b33e442ddea306fbfe00f9335e))

## [2.0.0](https://github.com/ScottGibb/Home-Lab-Containers/compare/v1.5.0...v2.0.0) (2026-02-08)

### ⚠ BREAKING CHANGES

* removing piweb

### Features

* Add Docker Compose to Dependabot configuration ([f436bfa](https://github.com/ScottGibb/Home-Lab-Containers/commit/f436bfa0ce2c3d2f005dd5eabb10b14b3caba07d))
* bump deps ([e21ac93](https://github.com/ScottGibb/Home-Lab-Containers/commit/e21ac93688672964a765bf4b64a0a300870e6a8f))
* removing piweb ([b1a0f30](https://github.com/ScottGibb/Home-Lab-Containers/commit/b1a0f3023a9ad6f03c597a4964d8626ae11c6331))
* use prebuilt image for presence detector ([e43103b](https://github.com/ScottGibb/Home-Lab-Containers/commit/e43103bad0c76dcba48932be4625e825a729d0fa))

### Bug Fixes

* dependabot ([993f0f9](https://github.com/ScottGibb/Home-Lab-Containers/commit/993f0f97be16c60e4e18701c7dcccbe80a909d4d))
* docker path for smart rf transmitter ([23916b9](https://github.com/ScottGibb/Home-Lab-Containers/commit/23916b9b3ee3c3ea0454d4ded80be52707b43aa9))
* **docs:** naming conventions ([37e862d](https://github.com/ScottGibb/Home-Lab-Containers/commit/37e862d42bd5970370f0413f6b137a38e59d1abd))
* **docs:** remove png ([150649c](https://github.com/ScottGibb/Home-Lab-Containers/commit/150649cdd51e915a9c0659d4a4de55e8787451fb))
* tty issues with cmdline tui ([afeb7c2](https://github.com/ScottGibb/Home-Lab-Containers/commit/afeb7c227cbe23b597f6528686fe5c0092d41393))

## [1.5.0](https://github.com/ScottGibb/Home-Lab-Containers/compare/v1.4.0...v1.5.0) (2025-12-15)

### Features

* **ci:** Update Dependabot workflow for auto-merging ([a5f691f](https://github.com/ScottGibb/Home-Lab-Containers/commit/a5f691fa971a8ab964d132e02d44d61aa865faf4))

### Bug Fixes

* **ci:** Add directory scope to Dependabot ([e371e89](https://github.com/ScottGibb/Home-Lab-Containers/commit/e371e89a8e58d6056e484bdef2eaa58b80393beb))

## [1.4.0](https://github.com/ScottGibb/Home-Lab-Containers/compare/v1.3.2...v1.4.0) (2025-12-02)

### Features

* Remove gitsubmodule configuration from dependabot ([ff35c81](https://github.com/ScottGibb/Home-Lab-Containers/commit/ff35c81247846c3d9ba8a8a238d1922dab837b05))

### Bug Fixes

* **ci:** Add Dependabot configuration ([701af8b](https://github.com/ScottGibb/Home-Lab-Containers/commit/701af8b39bc8cfd6e901a247103f9444557551b7))
* **ci:** Dependabot ([b351ab4](https://github.com/ScottGibb/Home-Lab-Containers/commit/b351ab4fed84e57a46fdc400b925cb273f6793fa))
* **ci:** fix dependabot again ([27e45ae](https://github.com/ScottGibb/Home-Lab-Containers/commit/27e45aea47cb0d9bf48eb56b5a9a79ab39d33ca4))

## [1.3.2](https://github.com/ScottGibb/Home-Lab-Containers/compare/v1.3.1...v1.3.2) (2025-11-30)

### Bug Fixes

* **ci:** fix dependabot naming ([be38fed](https://github.com/ScottGibb/Home-Lab-Containers/commit/be38fed4d5f82b02cbd71e8f1dc9b7d6d11be27c))
