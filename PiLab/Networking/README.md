...
- **NGINX_HOST**: The IP address of this system (192.168.0.69)
...
To start the networking services for this system, run from the PiLab directory:

```bash
docker compose up -d
```

The main `docker-compose.yml` file in the PiLab directory includes both the common networking configuration from `utils/Networking` and the system-specific settings from this folder's `.env` file.
